...
class NERServiceFactory:
    ...
    def create_service(cls, provider: str, **kwargs) -> NERServiceInterface:
        ...
        if normalized_provider not in cls._service_registry:
            ...
            raise ValueError(
                f"Unsupported NER provider: {provider}. Available providers: {available}"
            )
    ...
    def create_from_settings(
        ...
        return cls.create_service(
            runtime_settings.ner_provider, settings=runtime_settings
        )
    ...
    def create_from_env(cls) -> NERServiceInterface:
        ...
        provider = (
            os.getenv("VOICEFLOW_NER_PROVIDER") or os.getenv("NER_PROVIDER") or "spacy"
        )
        return cls.create_service(provider, settings=Settings())

    @classmethod
    def register_service(
        cls, name: str, service_class: Type[NERServiceInterface]
    ) -> None:
