...
class VoiceFlowTester:
        ...
        try:
            ...
            if minimal:
                ...
                response = client.chat.completions.create(
                    model="gpt-4",
                    messages=[{"role": "user", "content": "Responde 'Test exitoso' en español"}],
                    max_tokens=30
                )
    ...
    async def test_azure_speech(self, minimal: bool = True) -> bool:
        ...
        try:
            ...
            if not minimal:
                self.print_step("Nota: Test completo de audio requiere entrada de micrófono", "WARNING")
    ...
    def print_final_report(self):
        ...
        components = [
            ...
            ('Azure Speech Services', self.results.get('azure_speech_connection', False)),
        ...
        # Guardar resultados
        with open(f"test_results_{self.results.get('mode')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", 'w', encoding='utf-8') as f:
...
async def main():
    ...
    parser.add_argument('--prod', action='store_true', help='Ejecutar en modo producción (test completo)')
    ...
    try:
        ...
        if mode == 1:
            results = await tester.run_test_mode()
        else:
            results = await tester.run_production_mode()
