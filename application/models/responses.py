...
class AudioTranscriptionResponse(BaseResponse):
    ...
    is_simulation: bool = Field(
        default=False, description="Whether this is a simulated response"
    )
...
class AudioProcessingStatusResponse(BaseResponse):
    ...
    progress: Optional[float] = Field(
        default=None, description="Processing progress (0-1)"
    )
...
class PipelineStep(BaseModel):
    ...
    status: str = Field(
        default=PipelineStatus.PENDING, description="pending|processing|completed|error"
    )
    duration_ms: Optional[int] = Field(
        default=None, description="Processing time in milliseconds"
    )
    summary: Optional[str] = Field(
        default=None, description="Brief summary of step output"
    )
...
class ChatResponse(BaseResponse):
    ...
    ai_response: Optional[str] = Field(
        default=None, description="AI assistant response"
    )
    ...
    tourism_data: Optional[TourismData] = Field(
        default=None, description="Structured tourism information"
    )
