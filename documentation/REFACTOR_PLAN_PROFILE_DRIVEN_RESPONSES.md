...
1. **Enriquecer método `get_profile_context()`**

```python
def get_profile_context(self, profile_id: str) -> Optional[dict]:
    """Get enriched profile context with ranking and filtering rules."""
    ...
    # Extract expected types (bias > 1.0)
    expected_types = [
        vtype for vtype, bias in venue_types.items() if bias > 1.0
    ]
    
    # Build filter rules
    filter_rules = {
        "min_score": 5.0,  # Default min accessibility score
        "exclude_types": [],
...
**Puntos críticos:**
- Nueva funcionalidad: extracción de `expected_types`
- Logs adicionales para observabilidad
